...
export default function Scoring() {
  ...
  return (
    <div className="flex bg-surface font-body text-on-surface antialiased min-h-screen">
      ...
      {/* TopAppBar */}
      <header className="flex justify-between items-center w-full px-8 py-4 h-20 fixed top-0 z-30 bg-white/70 backdrop-blur-xl border-b border-slate-100 dark:border-slate-800 ml-64" style={{ width: 'calc(100% - 16rem)' }}>
        <div className="flex items-center gap-6">
          <h2 className="text-lg font-extrabold text-slate-900 font-headline">Nexus Management</h2>
          <div className="relative focus-within:ring-2 focus-within:ring-blue-500/20 rounded-full">
            <span className="material-symbols-outlined absolute left-4 top-1/2 -translate-y-1/2 text-slate-400">search</span>
            <input className="bg-surface-container-low border-none rounded-full pl-12 pr-6 py-2 w-80 text-sm focus:bg-white focus:ring-0 transition-all outline-none" placeholder="Search teams or judges..." type="text" />
          </div>
        </div>
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-3 ml-2 pl-4 border-l border-slate-200">
            ...
            <img className="w-10 h-10 rounded-full object-cover" src="https://lh3.googleusercontent.com/aida-public/AB6AXuBtMqDMUaBBOwj-2gvbLH2Vatc0ChKSJQ4K6HJX5Vi42CdICGqPagIBhXpza_MqUrauzU_81qlgSmkqhBGlBKXRQB7Ot49DLR6limNiITTQA2DUpQNPlrYIgHe2uD394wi0e1_NPpTE47bXq1V8Xtmon7qlJBf0fBxHGXc0OQC7qW0gWUmM3BLRzkSDNzkm-mojkfr4lQDikCMiI2LggQAcggpE8ly1Ci4x5RwXTuEVPk8G4EDgv_DfyRI20vN8FU9S104E61CseeNs" alt="Admin Avatar" />
          </div>
        </div>
      </header>

      {/* Main Content Canvas */}
      <main className="flex-1 ml-64 pt-24 min-h-screen p-8">
        {/* Header Section */}
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10">
          <div>
            <nav className="flex gap-2 text-xs font-semibold text-primary mb-2">
              <span>Events</span>
              <span>/</span>
              <span className="text-slate-400">Global Innovation Summit 2024</span>
            </nav>
            <h3 className="text-4xl font-extrabold font-headline text-on-surface tracking-tight">Scoring Overview</h3>
          </div>
          <div className="flex items-center gap-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
          <div className="bg-surface-container-lowest p-8 rounded-xl flex flex-col justify-between shadow-sm border border-slate-100 dark:border-slate-800 relative overflow-hidden group">
            <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
              <span className="material-symbols-outlined text-6xl text-primary">leaderboard</span>
            </div>
            <div>
              <p className="text-slate-500 font-medium text-sm mb-1 font-label">Average Event Score</p>
              <h4 className="text-4xl font-black font-headline text-primary tracking-tighter">
                84.2<span className="text-lg text-slate-400 font-body font-bold">/100</span>
              </h4>
            </div>
          ...
          <div className="bg-surface-container-lowest p-8 rounded-xl flex flex-col justify-between shadow-sm border border-slate-100 dark:border-slate-800 relative overflow-hidden group">
            ...
            <div>
              <p className="text-slate-500 font-medium text-sm mb-1 font-label">Evaluation Progress</p>
              <h4 className="text-4xl font-black font-headline text-on-surface tracking-tighter">78%</h4>
            </div>
            <div className="mt-6 w-full bg-surface-container-low h-2 rounded-full overflow-hidden">
              <div className="h-full bg-gradient-to-r from-[#0052cc] to-[#003d9b]" style={{ width: '78%' }}></div>
            </div>
            <p className="mt-2 text-[10px] font-bold text-slate-400 uppercase tracking-widest font-label">156 / 200 Teams Scored</p>
          </div>
          ...
          <div className="bg-primary-container p-8 rounded-xl flex flex-col justify-between shadow-xl relative overflow-hidden text-white">
            ...
            <div>
              <p className="text-blue-100 font-medium text-sm mb-1 font-label">Pending Discrepancies</p>
              <h4 className="text-4xl font-black font-headline tracking-tighter">12</h4>
            </div>
        ...
        <div className="bg-surface-container-lowest rounded-xl shadow-sm border border-slate-100 dark:border-slate-800 overflow-hidden">
          <div className="p-6 border-b border-surface-container-low flex justify-between items-center bg-white dark:bg-slate-900">
            <h5 className="font-bold text-lg font-headline">Live Scoring Leaderboard</h5>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-surface-container-low/50">
                  <th className="px-6 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider font-label">Team Name</th>
                  <th className="px-6 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider font-label">Total Score</th>
                  <th className="px-6 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider font-label">Breakdown (P/A/F/U/I)</th>
                  <th className="px-6 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider font-label">Assigned Judges</th>
                  <th className="px-6 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider font-label">Status</th>
                  <th className="px-6 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider text-right font-label">Actions</th>
                </tr>
              </thead>
                  ...
                  <tr key={idx} className="hover:bg-surface-container-low/30 transition-colors group">
                    <td className="px-6 py-5">
                      <div className="flex items-center gap-3">
                        <div className={`w-10 h-10 rounded-lg ${row.color} flex items-center justify-center font-bold font-headline`}>
                          {row.initials}
                        </div>
                        <div>
                          <p className="font-bold text-slate-900 dark:text-slate-100">{row.teamName}</p>
                          <p className="text-[10px] text-slate-500 break-words max-w-[120px] leading-tight mt-0.5">{row.category}</p>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-5">
                      <span className={`font-black text-lg font-headline ${row.score === '--.-' ? 'text-slate-400' : 'text-primary'}`}>
                        {row.score}
                      </span>
                    </td>
                    <td className="px-6 py-5">
                      <div className="flex gap-1">
                        {['P', 'A', 'F', 'U', 'I'].map(key => (
                          <div key={key} className="w-8 h-8 rounded bg-slate-100 dark:bg-slate-800 flex flex-col items-center justify-center" title={key}>
                            <span className="text-[8px] font-bold text-slate-400 uppercase font-headline">{key}</span>
                            <span className={`text-[10px] font-bold ${row.score === '--.-' ? 'text-slate-300 dark:text-slate-600' : 'text-slate-700 dark:text-slate-300'} font-label`}>
                              {row.breakdown[key]}
                            </span>
                          </div>
                        ))}
                    ...
                    <td className="px-6 py-5">
                      <div className="flex -space-x-2">
                        {row.judges.map((img, i) => (
                          <img key={i} className="w-8 h-8 rounded-full border-2 border-white dark:border-slate-900 object-cover" src={img} alt="Judge" />
                        ))}
                        {row.extraJudges && (
                    ...
                    <td className="px-6 py-5">
                      <span className={`px-3 py-1 rounded-full text-[10px] font-bold ${row.statusColor} uppercase tracking-wide`}>
                        {row.status}
                      </span>
                    </td>
          ...
          <div className="p-6 bg-surface-container-low/20 flex justify-between items-center border-t border-slate-100 dark:border-slate-800">
            <p className="text-xs font-medium text-slate-500">Showing 1-4 of 200 teams</p>
            <div className="flex gap-2">
              ...
              <button className="w-10 h-10 rounded-lg flex items-center justify-center bg-primary-container text-white shadow-md font-bold text-sm">1</button>
              <button className="w-10 h-10 rounded-lg flex items-center justify-center bg-white dark:bg-slate-900 shadow-sm text-slate-600 dark:text-slate-300 font-bold text-sm hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors border border-slate-100 dark:border-slate-800">2</button>
              <button className="w-10 h-10 rounded-lg flex items-center justify-center bg-white dark:bg-slate-900 shadow-sm text-slate-600 dark:text-slate-300 font-bold text-sm hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors border border-slate-100 dark:border-slate-800">3</button>
              <button className="w-10 h-10 rounded-lg flex items-center justify-center bg-white dark:bg-slate-900 shadow-sm text-slate-400 hover:text-primary transition-colors border border-slate-100 dark:border-slate-800">
      ...
      <button className="fixed bottom-10 right-10 w-16 h-16 rounded-full bg-gradient-to-r from-[#0052cc] to-[#003d9b] text-white shadow-2xl flex items-center justify-center hover:scale-110 transition-transform active:scale-95 group z-50 cursor-pointer">
        ...
        <span className="absolute right-full mr-4 bg-slate-900 text-white text-xs font-bold py-2 px-4 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap pointer-events-none">New Scoring Audit</span>
      </button>
    </div>
  );
}
